...
import { Home, Zap, Activity, User, Settings, Lock, Moon, Sun, Globe, ArrowLeft, ChevronRight, Sparkles, Instagram, Send, Users, CalendarHeart, Utensils, Scale, Dumbbell, Flame, Plus, Trash2, CreditCard, HelpCircle, X, ShieldCheck } from 'lucide-react';
import './App.css';
...
const T = {
  uk: {
    ...
    // MENU
    menu_empty: "Меню ще не готове", menu_soon: "Слідкуй за оновленнями!",
    menu_buy: "Купити меню", manage_menu: "Керування Меню",
    menu_desc: "Опис меню", menu_price: "Ціна (грн)"
  },
  en: {
    ...
    // MENU
    menu_empty: "Menu not ready yet", menu_soon: "Stay tuned!",
    menu_buy: "Buy Menu", manage_menu: "Manage Menu",
    menu_desc: "Menu Description", menu_price: "Price (UAH)"
  }
};
...
function App() {
  ...
  const [viewMenu, setViewMenu] = useState(false); // Открыто ли меню

  // ADMIN STATES
  const [adminTab, setAdminTab] = useState('news'); // news | menu
              ...
              <motion.div key="admin" className="fullscreen-page" initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}} transition={{type:"spring", damping:25}}>
                ...
                <div className="scroll-content">
                  ...
                  {adminTab === 'news' ? (
                    <>
                      <div className="admin-section">
                        ...
                        <div className="color-picker">
                          {NEWS_THEMES.map((th, i) => <motion.div key={i} className={`color-btn ${newArticle.themeIdx===i?'active':''}`} style={{background:th.bg}} onClick={()=>setNewArticle({...newArticle, themeIdx:i})}/>)}
                        </div>
                        <div className="input-group">
                    ...
                    // УПРАВЛЕНИЕ МЕНЮ
                    <div className="admin-section">
                      <div className="admin-title">{t('manage_menu')}</div>
                      {menuData ? (
                        <div className="menu-row" style={{flexDirection:'column', alignItems:'flex-start'}}>
                        ...
                        <>
                          <div className="input-group">
                            <input className="custom-input" placeholder="Назва меню" value={newMenu.title} onChange={e=>setNewMenu({...newMenu, title:e.target.value})}/>
                          </div>
                          <div className="input-group">
                            <textarea className="custom-input" placeholder={t('menu_desc')} value={newMenu.desc} onChange={e=>setNewMenu({...newMenu, desc:e.target.value})}/>
                          </div>
                          <div className="input-group">
                            <input className="custom-input" type="number" placeholder={t('menu_price')} value={newMenu.price} onChange={e=>setNewMenu({...newMenu, price:e.target.value})}/>
                          </div>
                ...
                <div className="scroll-content">
                  {menuData ? (
                    <>
                      ...
                      <motion.button whileTap={{scale:0.95}} className="action-btn"><CreditCard size={20}/> {t('menu_buy')} — {menuData.price} ₴</motion.button>
                    </>
                  ) : (
                    <div className="glass-card" style={{width:'100%'}}>
                      <div className="icon-glow-container" style={{background:'#ccc'}}><Lock size={50}/></div>
                      <h3>{t('menu_empty')}</h3>
                      <p>{t('menu_soon')}</p>
                    </div>
                  )}
